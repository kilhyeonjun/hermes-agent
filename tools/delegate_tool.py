...
def delegate_task(
    ...
    *,
    ...
    if tasks and isinstance(tasks, list):
        ...
        task_list = [{"goal": goal, "context": context, "role": top_role, "preset": preset}]
    else:
    ...
    # Resolve every unique operator preset before constructing any child.
    route_cache: Dict[Optional[str], tuple] = {}
    resolved_routes: List[tuple] = []
    ...
    if background:
        ...
        if _unique_presets == {None}:
            # Preserve legacy metadata when no preset is selected. Global
            # delegation.model may still be configured and used by every child.
            _metadata_model = resolved_routes[0][2].get("model")
        elif len(_unique_presets) == 1:
            # Do not overload the async `model` metadata field with a preset
            # name, and do not reveal the resolved model back to the parent.
...
def _resolve_delegation_preset_config(cfg: dict, preset_name: Optional[str]) -> dict:
    ...
    for key in _PRESET_OVERRIDE_KEYS:
        if key in preset:
            base[key] = preset[key]
    if "provider" in preset and str(preset.get("provider") or "").strip():
        # A trusted preset provider is an explicit route selection. Global
        # direct-endpoint settings otherwise take precedence inside
        # _resolve_delegation_credentials() and would silently defeat it.
        base["base_url"] = ""
        base["api_key"] = ""
        base["api_mode"] = ""
    return base
...
def _build_top_level_description() -> str:
    ...
    if _get_delegation_presets():
        model_routing_clause = (
            "- Raw subagent model/provider/reasoning values are NOT selectable "
            "per call. Only operator-configured preset names may be selected; "
            "omit preset to inherit the global delegation route and then the parent.\n"
        )
    else:
        model_routing_clause = (
            ...
            "pinned via delegation.provider / delegation.model in config.yaml.\n"
        )
...
def _build_dynamic_schema_overrides() -> dict:
    ...
    actual limits.
    """
    overrides_params: Dict[str, Any] = copy.deepcopy(DELEGATE_TASK_SCHEMA["parameters"])
