...
from utils import atomic_write_text as _utils_atomic_write_text, is_truthy_value
...
def atomic_write_text(file_path: Path, content: str, encoding: str = "utf-8") -> None:
    """Atomically write within a configured skills root."""
    _assert_within_skills_roots(file_path)
    _utils_atomic_write_text(file_path, content, encoding=encoding)
...
def skill_manage(
    ...
    # atomic_write_text, which raises when the target escapes the skills roots
