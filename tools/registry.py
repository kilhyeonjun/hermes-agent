...
class ToolRegistry:
    ...
    def mutation_transaction(self):
        ...
        The registry lock is re-entrant, so callers may use normal register,
        deregister, and alias APIs inside the context. Readers cannot observe a
        partial surface. On failure the entries, toolset checks, and aliases are
        restored; generation remains monotonic to invalidate stale snapshots.
        """
        with self._lock:
