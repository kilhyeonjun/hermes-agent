...
from gateway.platforms.base import EphemeralReply, MessageEvent, MessageType, PickerCallbackOutcome
...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_codex_account_command(self, event: MessageEvent) -> Optional[str]:
        ...
        native_account_script = get_default_hermes_root() / "scripts" / "codex_native_account.py"
        try:
        ...
        async def _execute(selected_mode: str) -> PickerCallbackOutcome:
            ...
            try:
                ...
                return PickerCallbackOutcome(f"❌ Codex 계정 명령 실패: {safe_exc}", status="failure")

            output = redact_sensitive_text(
                (proc.stdout or proc.stderr or "출력 없음").strip(), force=True
            )
        ...
        if adapter is None or not hasattr(adapter, "send_codex_account_picker"):
            return status_card
        match = re.search(
            r"Native Codex CLI:\s*(personal|company|none|legacy|unknown)", status_card
        )
