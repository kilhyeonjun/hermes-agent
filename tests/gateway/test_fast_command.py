...
def test_turn_route_uses_explicit_session_normal_over_shared_fast_state():
    runner = _make_runner()
    runner._service_tier = "priority"
    runtime_kwargs = {
        "api_key": "***",
...
@pytest.mark.asyncio
async def test_run_agent_passes_session_priority_processing_to_gateway_agent(monkeypatch, tmp_path):
    _install_fake_agent(monkeypatch)
    runner = _make_runner()
    runner._session_service_tier_overrides = {
        "agent:main:telegram:dm:12345": "priority"
    }
    ...
    monkeypatch.setattr(
        ...
        lambda: {"agent": {"service_tier": "normal"}},
    )
    monkeypatch.setattr(gateway_run, "_resolve_gateway_model", lambda config=None: "gpt-5.4")
    monkeypatch.setattr(
        gateway_run,
